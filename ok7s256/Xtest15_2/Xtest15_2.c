/* ========================================================================== */
/*	 Xtest15_2.c : AT93C56 Read/Write(ORG=1, 128x16 bit, 8-bit SPI)       */
/* ========================================================================== */
...
void AT93C56_EWEN(void)		                /* write enable */
{
  ...
  AT91F_SPI_PutChar(AT91C_BASE_SPI, 0x04, 1);	// 0000 0100
  while(!(*AT91C_SPI_SR & 0x0200));		// transmit complete ?
  AT91F_SPI_PutChar(AT91C_BASE_SPI, 0xC0, 1);	// 1100 0000
  while(!(*AT91C_SPI_SR & 0x0200));		// transmit complete ?
...
void AT93C56_WRITE(unsigned int address,unsigned int byte2) /* write 16 bit */
{
  AT91F_PIO_SetOutput(AT91C_BASE_PIOA, CS1);	// CS = 1

  AT91F_SPI_PutChar(AT91C_BASE_SPI, 0x05, 1);	// 0000 0101
  while(!(*AT91C_SPI_SR & 0x0200));		// transmit complete ?
  AT91F_SPI_PutChar(AT91C_BASE_SPI, address, 1); // A7~A0
  while(!(*AT91C_SPI_SR & 0x0200));		// transmit complete ?
  AT91F_SPI_PutChar(AT91C_BASE_SPI, byte2 >> 8, 1);     // D15~D8
  while(!(*AT91C_SPI_SR & 0x0200));		// transmit complete ?
  AT91F_SPI_PutChar(AT91C_BASE_SPI, byte2 & 0x00FF, 1);	// D7~D0
  while(!(*AT91C_SPI_SR & 0x0200));		// transmit complete ?
...
{ unsigned int upper, lower;
  ...
  AT91F_SPI_PutChar(AT91C_BASE_SPI, 0x06, 1);	// 0000 0110
  while(!(*AT91C_SPI_SR & 0x0200));		// transmit complete ?
  AT91F_SPI_PutChar(AT91C_BASE_SPI, address, 1); // A7~A0
  while(!(*AT91C_SPI_SR & 0x0200));		// transmit complete ?
  AT91F_SPI_PutChar(AT91C_BASE_SPI, 0x00, 1);	// dummy data
  while(!(*AT91C_SPI_SR & 0x0200));		// transmit complete ?
  upper = AT91F_SPI_GetChar(AT91C_BASE_SPI);
  AT91F_SPI_PutChar(AT91C_BASE_SPI, 0x00, 1);	// dummy data
  while(!(*AT91C_SPI_SR & 0x0200));		// transmit complete ?
  lower = AT91F_SPI_GetChar(AT91C_BASE_SPI);

  AT91F_PIO_ClearOutput(AT91C_BASE_PIOA, CS1);	// CS = 0
  return (upper << 8) + lower;			// get data and return
}
...
int main(void)
{
  ...
  *AT91C_SPI_MR = 0x00010001;			// master mode
  AT91F_SPI_CfgCs(AT91C_BASE_SPI, 1, 0x02003002); // CS1,1M bps,8 bit,NCPHA=1,CPOL=0
