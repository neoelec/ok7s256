...
void GLCD_s2d(signed int number)		/* display signed 2-digit decimal number */
{
  if(number >= 0)				// sign
    GLCD_English(0,'+');
  else
...
void GLCD_s4d(signed int number)		/* display signed 4-digit decimal number */
{
  if(number >= 0)				// sign
    GLCD_English(0,'+');
  else
  ...
  while(1)
    { GLCD_string(0,0,1," 부호 없는 정수 ");	// multiplication without sign
      ...
      for(i = 10; i <= 90; i += 10)		// multiplication without sign
        for(j = 10; j <= 90; j += 10)
          { GLCD_xy(1,1); GLCD_2d(i);		// display multiplicand
            GLCD_xy(1,6); GLCD_2d(j);		// display multiplier
            GLCD_xy(1,11); GLCD_4d(i*j);	// display multiplication
            Delay_ms(1000);
          }
      ...
      for(x = -90; x <= +90; x += 10)		// multiplication with sign
        for(y = -90; y <= +90; y += 10)
          { GLCD_xy(3,0); GLCD_s2d(x);		// display multiplicand
            GLCD_xy(3,5); GLCD_s2d(y);		// display multiplier
            GLCD_xy(3,11); GLCD_s4d(x*y);	// display multiplication
            Delay_ms(1000);
          }
    }
}
