...
void GLCD_2BCD(unsigned int number)		/* display 2-digit BCD number */
{
...
void RTC_display(void)				/* display clock from RTC */
{
  ...
  GLCD_xy(1,9); GLCD_2BCD(date);

  weekday = TWI_receive(0x03);			// display weekday
  if(weekday == 1)      GLCD_string(1,12,0,"일");
  else if(weekday == 2) GLCD_string(1,12,0,"월");
  else if(weekday == 3) GLCD_string(1,12,0,"화");
  else if(weekday == 4) GLCD_string(1,12,0,"수");
  else if(weekday == 5) GLCD_string(1,12,0,"목");
  else if(weekday == 6) GLCD_string(1,12,0,"금");
  else                  GLCD_string(1,12,0,"토");

  hour = TWI_receive(0x02) & 0x3F;		// display hour
  if(hour > 0x20) { hour &= 0x1F; ampm = 'P';}
  else            ampm = 'A';
  GLCD_xy(2,2); GLCD_2BCD(hour);
  GLCD_xy(2,11); GLCD_English(0,ampm);		// display AM/PM
  GLCD_xy(2,12); GLCD_English(0,'M');

  minute = TWI_receive(0x01);			// display minute
  GLCD_xy(2,5); GLCD_2BCD(minute);

  second = TWI_receive(0x00);			// display second
  GLCD_xy(2,8); GLCD_2BCD(second);

  GLCD_xy(2,15); GLCD_English(0,' ');		// display home position
...
{
  if((xcursor == 2) && (ycursor == 15))      ycursor = 12;
  else if((xcursor == 2) && (ycursor == 12)) ycursor = 9;
  else if((xcursor == 2) && (ycursor == 9))  ycursor = 6;
  else if((xcursor == 2) && (ycursor == 6))  ycursor = 3;
  else if((xcursor == 2) && (ycursor == 3)) {xcursor = 1; ycursor = 12;}
  else if((xcursor == 1) && (ycursor == 12)) ycursor = 10;
  else if((xcursor == 1) && (ycursor == 10)) ycursor = 7;
  else if((xcursor == 1) && (ycursor == 7))  ycursor = 4;
  else if((xcursor == 1) && (ycursor == 4)) {xcursor = 2; ycursor = 15;}
}
