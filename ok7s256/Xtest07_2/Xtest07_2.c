...
void GLCD_2hex(unsigned int number)		/* display 2-digit hex number */
{ unsigned int i;

  i = (number >> 4) & 0x0000000F;		// 16^1
  if(i <= 9) GLCD_English(0,i + '0');
  else       GLCD_English(0,i - 10 + 'A');
...
void GLCD_8bin(unsigned int number)		/* display 8-bit binary number */
{
